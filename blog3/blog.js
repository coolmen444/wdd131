...
for (const article of articles) {
	newContent += `<aside class="book-info"><time datetime="${article.date}">${article.date}</time><p>${article.ages}</p><p>${article.genre}</p><p>${article.stars}</p></aside><article><h2 class="book-title">${article.title}</h2><img src="${article.imgSrc}" alt="${article.imgAlt}"><p >${article.description}</p></article><br>`;
}
mainSection.innerHTML = newContent;
mainSection.innerHTML += `<aside id="filter-panel">
